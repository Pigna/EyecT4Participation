...
namespace EyeCT4Participation.Database
{
    class DBaccount : DB
    {
        ...
        public int GetQueryLogIn(string username, string password) //name of ur query
        {
            ...
            List<Dictionary<string, object>> LogInQuery = getQuery("SELECT Type FROM Gebruiker WHERE Gebruikersnaam = " + username + "AND Wachtwoord =" + password + ";"); //replace your query with te example query, replace 'QueryX' with a clear name.
